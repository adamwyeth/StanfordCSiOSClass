...
class CalculatorBrain
{
    ...
    private func createFunction(ops: [Op]) -> (fun: (Double -> Double)?, remainingOps: [Op])
    {
        if !ops.isEmpty {
            var remainingOps = ops
            let op = remainingOps.removeLast()
            switch op {
            case .Operand(let operand):
                return ({(s1: Double) -> Double
                    in operand}, remainingOps)
            case .ConstantOperation(_, let constant):
                return ({ (s1: Double) -> Double in
                    constant}, remainingOps)
            case .VariableOperation(let symbol):
                return ({ (s1: Double) -> Double in
                    s1 }, remainingOps)
            case .UnaryOperation(_, let operation):
                let eval = createFunction(remainingOps)
                if let f = eval.fun
                {
                    return ({ (s1: Double) -> Double in
                        operation(f(s1))}, eval.remainingOps)
                }
            case .BinaryOperation(_, let operation):
                let f1Eval = createFunction(remainingOps)
                if let f1 = f1Eval.fun {
                    let f2Eval = createFunction(f1Eval.remainingOps)
                    if let f2 = f2Eval.fun {
                        return ({ (s1: Double) -> Double in
                            operation(f1(s1), f2(s1))}, remainingOps)
                    }
                }
            }
        }
    ...
    func createFunction() -> (Double -> Double)? {
        let (result, remainder) = createFunction(opStack)
        return result
    }
